...
spark = SparkSession.builder.appName("STEDI_joiner").getOrCreate()
...
valuesDf = redisDf.selectExpr("cast(value as string) value")
...
valuesDf.withColumn("value", from_json("value", redisServerSchema)) \
    .select(col("value.*")) \
...
encDf = spark.sql("select zSetEntries[0].element as customer from RedisSortedSet")
...
df.withColumn("customer", from_json("customer", customersSchema)).select(col("customer.*")).createOrReplaceTempView(
    "CustomerRecords")
...
jDf = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF, expr("""
    email = customer
"""))
...
jDf.selectExpr("to_json(struct(*)) AS value").writeStream \
    .outputMode("append") \
    .format("kafka") \
    .option("topic", "email-birth-score") \
    .option("kafka.bootstrap.servers", "kafka:19092") \
